...
// Get all Blogs page content (public)
export const getBlogsContent = async (req, res, next) => {
  try {
...
// Get all Blogs content for admin (including inactive)
export const getAllBlogsContent = async (req, res, next) => {
  try {
    const { data: content, error } = await supabase
      .from('website_content')
      .select('*')
...
// Bulk update Blogs content (for hero section)
export const bulkUpdateBlogsContent = async (req, res, next) => {
  try {
