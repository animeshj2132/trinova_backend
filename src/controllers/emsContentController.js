...
// Get all EMS page content (public)
export const getEMSContent = async (req, res, next) => {
  try {
...
// Get all EMS content for admin (including inactive)
export const getAllEMSContent = async (req, res, next) => {
  try {
    const { data: content, error } = await supabase
      .from('website_content')
      .select('*')
...
// Bulk update EMS content
export const bulkUpdateEMSContent = async (req, res, next) => {
  try {
